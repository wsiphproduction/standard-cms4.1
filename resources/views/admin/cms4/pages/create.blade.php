    ...
    <form method="post" action="{{ route('pages.store') }}" enctype="multipart/form-data">
        <div class="row row-sm">
            <div class="col-lg-6">
                ...
                <div class="form-group">
                    <label class="d-block">Parent Page</label>
                    <select id="parentPage" data-live-search="true" multiple="multiple" class="selectpicker mg-b-5 @error('parent_page_id') is-invalid @enderror form-control select2" name="parent_page_id[]" data-style="btn btn-outline-light btn-md btn-block tx-left" title="- None -" data-width="100%">
                        @forelse($pages as $page)
                            <option value="{{$page->id}}" {{ (old("parent_page_id") == $page->id ? "selected":"") }}> {{$page->name}} </option>
                        @empty
                        @endforelse
